...
class WhenFluentValidatorSpec extends Specification {
    ...
    def "when (simple)"() {
        ...
        where:
        car                                | result
        new Car(power: 50)                 | { c -> Fv.Result.success(c) } //skip by when
        new Car(title: "Lada", power: 150) | { c -> Fv.Result.success(c) } //correct title
        new Car(power: 150)                | { c -> Fv.Result.failure(c, Err.field("title").code("notEmpty").build()) }
    }
